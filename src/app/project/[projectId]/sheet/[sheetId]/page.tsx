...
export default function SheetPage() {
  ...
  // Load sheet data.
  // Critical path (gates the spinner): one bundled request → sheet meta + Excel rows.
  // Secondary data (waste inventory, generated waste, saved results) loads in
  // parallel and populates in the background so the page is interactive sooner.
  const loadSheetData = useCallback(async () => {
    ...
    Promise.all([genWasteP, resultsP])
      .then(([genData, resultsData]) => {
        const genArr = genData?.success ? (genData.waste ?? []) : [];
        setGeneratedWaste(genArr);
        if (resultsData?.success && resultsData.results?.length > 0) {
          const stats = buildWasteStatsFromDbResults(resultsData.results, genArr);
          if (stats) setSheetWasteStats(stats);
        }
      })
      .catch(() => { /* non-fatal: version display is optional */ });

    // ── Critical path: sheet meta + Excel rows in a single round trip ──
    try {
      const bundle = await bundleP;
      const sheet = bundle?.sheet;
      if (sheet) {
        setSheetInfo({ sheetNumber: sheet.sheetNumber, fileName: sheet.fileName, status: sheet.status });
        if (bundle.excel?.length) {
          setDisplayData(transformToDisplayFormat(sanitizeExcelData(bundle.excel)));
        }
      }
    } catch (err) {
