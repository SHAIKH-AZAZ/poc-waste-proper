...
export default function CuttingVisualization3D({
    ...
    const renderBarDiagram = (pattern: CuttingPattern, detailedCut: DetailedCut, barIndex: number) => {
        ...
        return (
            <div key={barIndex} className="mb-8 p-4 border border-gray-200 bg-white">
                {/* Bar Header */}
                <div className="flex justify-between items-center mb-4">
                    <div className="flex items-center gap-4">
                        <span className="text-lg font-bold text-gray-900">Bar {barIndex + 1}</span>
                ...
                <div className="relative mb-4">
                    {/* Bar Background */}
                    <div
                        className="relative bg-gradient-to-b from-gray-50 to-gray-100 border-2 border-gray-300 rounded-md shadow-sm"
                    ...
                    >
                        ...
                        <div className="absolute inset-0 flex">
                            {Array.from({ length: 12 }, (_, i) => (
                                <div
                                    key={i}
                                    className="border-r border-gray-400/50 flex-1 flex items-center justify-center text-xs font-medium text-gray-600"
                                >
                                    {i + 1}
                                </div>
                            ))}
                ...
                <div className="text-sm bg-gray-50 p-3 rounded-md border border-gray-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {detailedCut.cuts.map((cut, cutIndex) => (
                            <div key={cutIndex} className="flex items-center gap-3 p-2 bg-white rounded border border-gray-200 hover:border-gray-300 transition-colors">
                                <div
        ...
        <div className="w-full bg-white border border-gray-200 p-6">
            ...
            <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div className="flex items-center gap-3 p-2 bg-white rounded border border-blue-200">
                        ...
                        <span className="font-medium text-gray-700">Cut Segments</span>
                    </div>
                    <div className="flex items-center gap-3 p-2 bg-white rounded border border-blue-200">
                        <div className="w-5 h-5 bg-gradient-to-br from-red-500 to-red-600 rounded shadow-sm"></div>
                        <span className="font-medium text-gray-700">Waste Material</span>
                    </div>
                    <div className="flex items-center gap-3 p-2 bg-white rounded border border-blue-200">
                        <div className="w-2 h-5 bg-gradient-to-b from-red-500 to-red-600 rounded-full shadow-sm"></div>
                        <span className="font-medium text-gray-700">Lap Joint</span>
                    </div>
                    <div className="flex items-center gap-3 p-2 bg-white rounded border border-blue-200">
                        <div className="w-5 h-5 border-2 border-green-500 bg-green-100 rounded shadow-sm"></div>
                        <span className="font-medium text-gray-700">High Efficiency</span>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-6 pt-4 border-t-2 border-gray-200">
                <div className="mb-3 flex items-center gap-2">
                    ...
                    <span className="text-lg font-semibold text-gray-800">Optimization Summary</span>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="p-4 bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-lg">
                        <div className="text-2xl font-bold text-blue-700">{patterns.length}</div>
                        <div className="text-sm text-blue-600 font-medium">Total Bars</div>
                    </div>
                    <div className="p-4 bg-gradient-to-br from-red-50 to-red-100 border border-red-200 rounded-lg">
                        ...
                        <div className="text-sm text-red-600 font-medium">Total Waste</div>
                    </div>
                    <div className="p-4 bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-lg">
                        <div className="text-2xl font-bold text-green-700">
                            {(patterns.reduce((sum, p) => sum + p.utilization, 0) / patterns.length).toFixed(1)}%
                        </div>
                        <div className="text-sm text-green-600 font-medium">Avg Utilization</div>
                    </div>
                    <div className="p-4 bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-lg">
                        <div className="text-2xl font-bold text-purple-700">
                            {detailedCuts.reduce((sum, d) => sum + d.cuts.length, 0)}
                        </div>
                        <div className="text-sm text-purple-600 font-medium">Total Cuts</div>
                    </div>
                </div>
            </div>
        </div>
